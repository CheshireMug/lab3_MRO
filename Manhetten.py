...
# Вычисляем и выводим Манхэттенские расстояния между всеми парами точек
for i in range(len(x_coords)):
    for j in range(i + 1, len(x_coords)):
        dist = manhattan_distance((x_coords[i], y_coords[i]), (x_coords[j], y_coords[j]))
...
for class_num in set(classes):  # Уникальные номера классов
    ...
    plt.scatter(x_coords[indices], y_coords[indices],
